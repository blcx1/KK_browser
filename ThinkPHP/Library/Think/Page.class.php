...
class Page{
    ...
    public function show() {
        ...
        $this->url = str_replace('/index.php/','/',U(ACTION_NAME, $this->parameter));
        /* 计算分页信息 */
     ...
     * @return string
     */
    public function showForBootstrap() {
    	if(0 == $this->totalRows) return '';
    	...
    	for($i = 1; $i <= $this->rollPage; $i++){
    		...
    		if($page > 0 && $page != $this->nowPage){
    			...
    			if($page > 0 && $this->totalPages != 1){
    				$link_page .= '<li class="disabled" ><span> ' . $page . '</span></li>';
    			}
    		}
    	}
    ...
    public function show2() {
        ...
        //上一页
        $up_row  = $this->nowPage - 1;
        $up_page = $up_row > 0 ? '<a class="prev  xy" href="' . $this->url($up_row) . '">' . $this->config['prev'] . '</a>' : '';
    
        //下一页
        $down_row  = $this->nowPage + 1;
        $down_page = ($down_row <= $this->totalPages) ? '<a class="next xy" href="' . $this->url($down_row) . '">' . $this->config['next'] . '</a>' : '';
