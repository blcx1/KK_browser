...
class LanguageModel extends \Admin\Model\DefaultModel{
	...
	public function __destruct(){		
        ...
        public function getList(){
            $res=$this->field('name,iso_code')->where('is_delete=0')->order('id')->select();
            return $res;
        }
	...
	public function get_lang_list(){
		...
		if(check_array_valid($result)){
			...
			foreach($result as $value){
				...
				unset($iso_lang_list[$lang_id]['iso_code']);				
			}
	...
	public function info_add_process(){
		...
		$result = $this->where('iso_code = "'.addslashes($iso_code).'"')->count();
		if($result > 0){
     ...
     * 判断娱乐语言是否存在
     */
	public function istype($lang){
	    $sql="select * from tb_language where name='".$lang."'";
	    $res = $this->query($sql);
	    if($res){return $res;}
    }
     ...
     * 判断小说语言和分类是否对应存在
     */
    public function istype_book($lang,$type){
        $sql = "select a.*,b.id as tid,b.type_name from tb_language a join tb_book_type b on a.iso_code=b.language where a.name='$lang' and b.type_name='$type'";
        $res = $this->query($sql);
        return $res;
    }
     ...
     * 判断搞笑语言和分类是否对应存在
     */
    public function istype_funny($lang,$type){
        $sql = "select a.*,b.id as tid,b.type_name from tb_language as a right join tb_funny_type as b on a.iso_code=b.language where a.name='$lang' and b.type_name='$type'";
        $res = $this->query($sql);
        if($res){return $res;}
    }
     ...
     * 判断美女语言和分类是否对应存在
     */
    public function istype_beauty($type){
        $sql = "select a.*,b.id as tid,b.type_name from tb_language as a right join tb_beauty_type as b on a.iso_code=b.language where b.type_name='$type'";
        $res = $this->query($sql);
        if($res){return $res;}
    }
     ...
     * 判断游戏分类是否对应存在
     */
    public function istype_game($type){
        $sql = "select a.*,b.id as tid,b.type_name from tb_language as a right join tb_game_type as b on a.iso_code=b.language where b.type_name='$type'";
        $res = $this->query($sql);
        if($res){return $res;}
    }
     ...
     * 判断好物分类是否对应存在
     */
    public function istype_shop($type){
        $sql = "select a.*,b.id as tid,b.type_name from tb_language as a right join tb_shop_type as b on a.iso_code=b.language where b.type_name='$type'";
        $res = $this->query($sql);
        if($res){return $res;}
    }
     ...
     * 判断广告分类是否对应存在
     */
    public function istype_adver($type){
        $sql = "select a.*,b.id as tid,b.name from tb_language as a right join tb_web_navi as b on a.iso_code=b.language where b.name='$type'";
        $res = $this->query($sql);
        if($res){return $res;}
    }
